...
### 4. Hybrid kernel 修复的总结

> 下文用 `K` 指代 `num_speculative_tokens`（每步总草稿位预算），用 `S` 指代 `num_model_steps`（MTP 实际推的步数 / draft head 数）。Hybrid 模式下二者满足 `K > S`，差值 `K − S` 即为 ngram 需要补齐的草稿位数。每步 target 在最优情况下需要 verify `K + 1` 个 token（1 个上一步 verified + K 个 draft）。

PR 过程中发现并修复的关键 bug：`hybrid_mtp_ngram` kernel 将 `seq_lens_this_time` 写为 `S + 1 + ngram_hits`（其中 `0 ≤ ngram_hits ≤ K − S`，随真实 ngram 命中数变动），与 target cudagraph 在 capture 时按 `K + 1` 抓的 launch param 不一致[^1]。cudagraph replay 时下游 kernel（attention / append_kv 等）按 capture 时的 grid dim 启动，多出来的线程通过脏的 `cu_seqlens` / `slot_mapping` 寻址越界，引发 CUDA 700（mtp1+ngram2 的崩溃形态）或更隐蔽的内存腐败（mtp3+ngramX 的"看似正常但接受率被污染"）。

修复方案：在 hybrid kernel Phase 2 gather 末尾，**始终将 `seq_lens_this_time` pad 到 `K + 1`**，不足的位置（共 `K − S − ngram_hits` 个）以最后一个有效 draft token 作为 placeholder。target 会 verify 这些 placeholder 但拒绝它们，per-iter verify 数从可变变为固定，cudagraph 的 capture 与 replay 完全一致。该修复对正确性是必要的，对接受率统计也无副作用（placeholder 永远拒绝，不计入 accept），代价仅为每步多 verify 至多 `K − S` 个固定开销的 placeholder。

---

[^1]: **"target cudagraph 在 capture 时按 `K + 1` 抓"对应的代码链条**

    K+1 这个值是 capture 流程沿三段代码逐步写到 `seq_lens_this_time` 上的，然后被 cudagraph 内部固化进 launch param。

    **① capture 入口：MTP 路径专用、强制 accept_all_drafts**

    [gpu_model_runner.py:2097-2113](../fastdeploy/worker/gpu_model_runner.py#L2097-L2113)

    ```python
    elif self.speculative_decoding and self.spec_method in [
        SpecMethod.MTP, SpecMethod.SUFFIX, SpecMethod.NGRAM,
    ]:
        for capture_size in sorted(capture_sizes, reverse=True):
            ...
            self._dummy_run(
                ...
                accept_all_drafts=True,                    # ← 关键：让 sampler 强制接受全部 K 个 draft
            )
    ```

    **② slt 被实际写为 K+1：`speculate_step_paddle` op**

    [pre_and_post_process.py:788-814](../fastdeploy/model_executor/pre_and_post_process.py#L788-L814)

    ```python
    speculate_step_paddle(
        share_inputs["stop_flags"],
        share_inputs["seq_lens_this_time"],                # ← inplace 改这个
        ...
        share_inputs["accept_num"],                        # ← accept_all_drafts 让它 = K
        block_size,
        enc_dec_block_num,
        speculative_config.num_speculative_tokens,         # ← K 从这里进
    )
    ```

    每个 dummy iter 的 sampler 跑完后调用，读 `accept_num`（在 `accept_all_drafts=True` 下 = K）和 `num_speculative_tokens`（= K），把 `seq_lens_this_time` 写成 K+1，供下一个 iter 的 model forward 用。

    **③ cudagraph 实际"抓"的位置：dummy_run while 循环里的 model forward**

    [gpu_model_runner.py:2016-2037](../fastdeploy/worker/gpu_model_runner.py#L2016-L2037)

    ```python
    while True:
        self._prepare_inputs(is_dummy_or_profile_run=True)   # 读 share_inputs["seq_lens_this_time"]=K+1，算 cu_seqlens
        ...
        model_output = self.model(model_inputs, self.forward_meta)   # ← 走进 graph_optimization_backend
    ```

    `self.model(...)` 内部通过 [graph_optimization_backend.py](../fastdeploy/model_executor/graph_optimization/graph_optimization_backend.py) 拦截 forward，按当前 batch_size 选 bucket，调 paddle 的 cudagraph 录制 API，**把这一次 forward 里所有 kernel 的 launch param（grid dim、block dim、kernel args）固化进 graph node**。此时 slt = K+1，所以 cu_seqlens、grid dim、slot_mapping 写入数量等都按 K+1 烧进去。

    **完整时序**

    ```
    capture_model
     └─ _dummy_run(accept_all_drafts=True)
         └─ while True:
             iter 0 (prefill): slt = input_length, model() 不抓 decode cudagraph
             ├─ _dummy_sampler_run
             │   └─ post_process → speculate_step_paddle 把 slt 写成 K+1
             iter 1 (decode #1): slt = K+1, model() ← cudagraph 在这里抓到 K+1
             ├─ _dummy_sampler_run → speculate_step_paddle 仍写 K+1
             iter 2: slt = K+1, model() replay
             ...
             until seq_lens_this_time 全 0 退出
        ```
